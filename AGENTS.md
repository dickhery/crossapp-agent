...
- CrossApp Agent cannot fully OAuth as a remote MCP client on caffeine.ai domains: `mcp.beta.id.ai` only allows loopback redirects or vendor-allowlisted callback paths. In-app **Run now** uses browser → IC agents (ICRC-1 queries) under this app's II principal (zero backend cycles). Full act-as-you-at-NNS execution remains **Copy for MCP → Grok/Claude**.
