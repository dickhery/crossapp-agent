...
- The official IC MCP connector is `https://mcp.internetcomputer.org/mcp`. Plans must use real tool names (`canister_query`, `canister_update_call`, `resolve_app`, etc.), not invented ones like `icp.call_canister`. Keep frontend `MCP_CONNECTOR_URL` (`src/frontend/src/lib/mcp.ts`) and backend `OfflinePlan.MCP_URL` (`src/backend/lib/offline-plan.mo`) in sync.
...
- Product name in UI: **ICP MCP Connector Assistant** (`src/frontend/src/lib/brand.ts`). Backend canister stays `backend`.
...
- Pre-grant **read / write / execute** for Copy for MCP is also **localStorage-only** (`getGrantAllPermissionsDefault` / `setGrantAllPermissionsDefault` + `useAgentPermissions`). When on, `planClipboardPayload` embeds pre-confirmation language so the external agent need not wait for per-step approval; default remains confirm-before-writes. Do **not** put this on the canister — zero cycles and no migration.
- Memory `PreferredDApp` is `{ id, friendlyName, canisterIds : [Text], accountIds : [Text] }` (not a single `canisterId`). Changing this requires an enhanced-migration file under `src/backend/migrations/`. Cap lists in `core.mo` (`MAX_CANISTERS_PER_DAPP` / `MAX_ACCOUNTS_PER_DAPP`) so plan prompts stay small. Always include account IDs in `renderPreferences` / offline plans so MCP agents do not invent transfer destinations.
- This app is a **setup + workflow studio**, not an MCP executor. Users connect Grok/Claude to `https://mcp.internetcomputer.org/mcp`, draft plans here, then **Copy for MCP**.
